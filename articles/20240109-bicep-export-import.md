...
ネットサーフィンをしていたら Bicepの `@export()` と `import` という機能がプレビュー利用できるという記事[^1] を発見しました。これを利用することで Terraform っぽい変数分離ができそうなので試してみます。分離できると何が嬉しいのかというと、別のファイルで定義している変数を使いまわせるっていうことですね。Bicep では、ファイルの単位で変数が閉じられているので変数定義自体を共有することができないんですね。
