...
ACS の SMTP 認証は **SMTP AUTH（ユーザー名 + パスワード）** ですが、
その資格情報として **Microsoft Entra ID のアプリケーション登録（サービスプリンシパル）の情報**を利用します。
具体的には、SMTP パスワードは **Entra アプリのクライアント シークレット**であり、ACS 側がその情報を使って **Entra にトークンを取得**し、メール送信を承認します。
このため、既存の SMTP クライアントから ACS 経由で送信でき、かつ Entra による認証・認可の管理が可能になります。
